...
computeres:
  ...
  Lerror_t compute_g(Lfunc *L)
  {
    ...
    if((L->gprec == 0) && (L->target_prec==DEFAULT_TARGET_PREC) && (L->cache_dir)) // so cache_dir == NULL switches off caching
      ...
      fname2=(char *)malloc(1024*sizeof(char));
      fname1[0]=0;
      for(uint64_t r=0;r<L->degree;r++)
        {
	  snprintf(fname2, 1024,"%s_%.1f", fname1, L->mus[r]);
	  char *tmp=fname1;fname1=fname2;fname2=tmp;
	}
      snprintf(fname, 1337, "%s/g%s", L->cache_dir, fname1);
